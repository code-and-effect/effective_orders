    ...
    module Moneris
      ...
      def moneris_postback
        ...
        if @order.purchased?  # Fallback to a success condition of the Order is already purchased
          order_purchased(details: params, provider: 'moneris', redirect_url: purchased_redirect_url)
          return
        end
        ...
        if params[:result].to_s == '1' && params[:transactionKey].present?
          ...
          if response_code > 0 && response_code < 50  # Less than 50 means a successful validation
            order_purchased(details: params.merge(verify_params), provider: 'moneris', redirect_url: purchased_redirect_url)
          else
            order_declined(details: params.merge(verify_params), provider: 'moneris', redirect_url: declined_redirect_url)
          end
        else
          order_declined(details: params, provider: 'moneris', redirect_url: declined_redirect_url)
        end
      end
